<template>
  <div class="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 p-8">
    <div class="max-w-7xl mx-auto">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-4xl font-bold text-slate-900 mb-2">Manajemen WordCloud</h1>
        <p class="text-slate-600">Kelola semua wordcloud survei Anda</p>
      </div>

      <!-- Navigation -->
      <div class="mb-6 flex gap-4">
        <Link href="/admin/map" class="px-4 py-2 bg-slate-200 text-slate-900 rounded hover:bg-slate-300">
          Peta
        </Link>
        <Link href="/admin/wordcloud" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
          WordCloud
        </Link>
      ...
      <div class="bg-white rounded-lg shadow overflow-hidden">
        <div v-if="visualizations.length === 0" class="p-8 text-center text-slate-500">
          <p>Belum ada wordcloud.</p>
          <Link href="/admin/wordcloud/create" class="text-blue-600 hover:underline">Buat yang baru</Link>
        </div>
        <table v-else class="w-full">
          <thead class="bg-slate-100 border-b border-slate-200">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-sm font-semibold text-slate-900">Kata</th>
